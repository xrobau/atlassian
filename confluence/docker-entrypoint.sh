...
#
# If first CMD argument is not 'confluence' or blank, it will run that. Otherwise,
# it'll run confluence.

set -o errexit

[[ ${DEBUG} == true ]] && set -x

SERAPH_CONFIG_FILE="${PKG_INSTALL}/confluence/WEB-INF/classes/seraph-config.xml"
CROWD_PROPERTIES_FILE="${PKG_INSTALL}/confluence/WEB-INF/classes/crowd.properties"
...
function createConfluenceTempDirectory() {
  CONFLUENCE_CATALINA_TMPDIR=${PKG_INSTALL}/temp
...
function processConfluenceProxySettings() {
  if [ -n "${CONFLUENCE_PROXY_NAME}" ]; then
    xmlstarlet ed -P -S -L --insert "//Connector[not(@proxyName)]" --type attr -n proxyName --value "${CONFLUENCE_PROXY_NAME}" ${PKG_INSTALL}/conf/server.xml
  fi

  if [ -n "${CONFLUENCE_PROXY_PORT}" ]; then
    xmlstarlet ed -P -S -L --insert "//Connector[not(@proxyPort)]" --type attr -n proxyPort --value "${CONFLUENCE_PROXY_PORT}" ${PKG_INSTALL}/conf/server.xml
  fi

  if [ -n "${CONFLUENCE_PROXY_SCHEME}" ]; then
    xmlstarlet ed -P -S -L --insert "//Connector[not(@scheme)]" --type attr -n scheme --value "${CONFLUENCE_PROXY_SCHEME}" ${PKG_INSTALL}/conf/server.xml
  fi
}

function processContextPath() {
  if [ -n "${CONFLUENCE_CONTEXT_PATH}" ]; then
    xmlstarlet ed -P -S -L --update "//Context[contains(@docBase,'../confluence')]/@path" --value "${CONFLUENCE_CONTEXT_PATH}" ${PKG_INSTALL}/conf/server.xml
  fi
}

function relayConfluenceLogFiles() {
  TARGET_PROPERTY=1catalina.org.apache.juli.AsyncFileHandler.directory
  sed -i "/${TARGET_PROPERTY}/d" ${PKG_INSTALL}/conf/logging.properties
  echo "${TARGET_PROPERTY} = ${confluence_logfile}" >> ${PKG_INSTALL}/conf/logging.properties
...
function processCatalinaDefaultConfiguration() {
  if [ -f "${PKG_INSTALL}/bin/setenv.sh" ]; then
...
CATALINA_OPTS="-Dconfluence.context.path=${CONFLUENCE_CONTEXT_PATH} ${CATALINA_OPTS}"' >> ${PKG_INSTALL}/bin/setenv.sh
    echo "export CATALINA_OPTS" >> ${PKG_INSTALL}/bin/setenv.sh
  fi
}
...
function setCatalinaConfigurationProperty() {
  ...
  if [ -n "${configurationProperty}" ]; then
    sed -i "/${configurationProperty}/d" ${PKG_INSTALL}/bin/setenv.sh
    ...
    echo ${catalinaproperty} >> ${PKG_INSTALL}/bin/setenv.sh
  fi
}
...
if [ "$1" = 'confluence' -o ! "$!" ]; then
  exec ${PKG_INSTALL}/bin/start-confluence.sh -fg
else
  exec "$@"
fi
