...
function patchJar() {
	...
	sed -E -i \
		...
		-e '/return properties;/i // Debugging\nMap<String, String> env = System.getenv();\nfor (String envName : env.keySet()) {\n if (envName.startsWith("EXTRAS_")) {\n  System.out.format("DEBUG: --- OVERRIDE %s to %s%n", envName.substring(envName.indexOf("_")+1).replace("__","."),env.get(envName));\n properties.setProperty(envName.substring(envName.indexOf("_")+1).replace("__","."),env.get(envName)); }\n}' \
